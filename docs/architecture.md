...
- `gui/app.py` — `MainWindow` with toolbar (Open Project, Refresh, Generate, Stop, Output, View YAML, Archive), splitter layout, progress label, and signal wiring. Refresh reloads `project.yaml` from disk. Output opens an `OutputDialog` to choose between MP4 assembly and Kdenlive export. Archive opens an `ArchiveDialog` for the selected scene. Entry point: `run()`.
