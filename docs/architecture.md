<!-- Version: 2.13 | Last updated: 2026-03-15 -->
...
- `providers/fal.py` — FAL.ai (Flux 1.x, Flux 2, Kontext, O1 Image, Grok, Seedream, Hunyuan Image, Recraft, FireRed, Qwen Image, GLM Image, Nano Banana, Emu 3.5, GPT Image, Reve models for stills; Kling, Grok Video, Seedance, Hunyuan Video, Wan, MiniMax models for clips). Uses a `StillHandler` strategy pattern with model-family registry for still generation. Specialized handlers exist for models with unique argument construction (Ideogram, O1 Image, Kontext, Flux 2 Pro/Max, Instant Character), while `EditHandler` provides a generic configurable handler for models that share the `image_urls` + `/edit` endpoint pattern — configurable via `patterns`, `sizing`, `safety`, `supports_edit`, and `edit_suffix` params. The `edit_suffix` param (default `"/edit"`) controls which suffix is stripped during normalization and appended for edit routing; set to `None` for models where the model ID is the endpoint directly (e.g. FireRed, Qwen Edit), or to a non-standard suffix (e.g. `"/edit-image"` for Emu, `"/image-to-image"` for GLM). Flux 2 base models now route to `/edit` with `image_urls` when references are present (via EditHandler), replacing the previous Flux2Handler which had no edit support. The `_resolve_still_handler()` registry matches model IDs to handlers in specificity order, with `FluxHandler` as fallback. Kontext models auto-route to image-to-image (with reference) or text-to-image (without). Kling O3 clips and O1 Image stills support character elements and `@character_id` prompt rewriting (`@ElementN`/`@ImageN`). Kontext Max Multi accepts multiple references without explicit mapping. EditHandler checks both `scene_characters` and `reference_images` for refs, routing to the edit endpoint when either is present. Clip generation is decomposed into `_build_clip_args()` (model-specific argument building) and `_auto_route_clip_endpoint()` (t2v/i2v auto-routing), with model detection via `_is_grok_video`, `_is_seedance`, etc. Safety defaults are injected per handler before user options merge.
